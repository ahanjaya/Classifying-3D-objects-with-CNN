...
class Display:
    ...
    def filter_raw_data(self, data_pcl):
        ...
        data_x   = data_pcl[:,0]
        x_min = np.round( np.min(data_x), 1) + 0.1 # 0.5
        x_max = np.round( np.max(data_x), 1) - 0.3 # 0.3
        data_pcl = data_pcl[np.where( (data_x >= x_min) & (data_x <= x_max))]

        # 2nd filter (wall)
        data_y   = data_pcl[:,1]
        y_min = np.round( np.min(data_y), 1) + 0.1 # 0.2
        y_max = np.round( np.max(data_y), 1) - 0.1 # 1.2
        data_pcl = data_pcl[np.where( (data_y >= y_min) & (data_y <= y_max))]

        # 3nd filter (ground)
        data_z   = data_pcl[:,2]
        z_min = np.round( np.min(data_z), 1) + 0.08 # 0.08
