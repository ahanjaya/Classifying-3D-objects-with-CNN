...
def load_data(dp):
    ...
    label_dirs   = [ i for i in os.listdir(dp) if os.path.isdir(os.path.join(dp, i)) ]
    print(label_dirs)
    ...
    for i, dir_path in enumerate(label_dirs):
        for path in _train_test_split_paths( os.path.join(dp, dir_path), 'train'):
            train_paths.append(path)
            y_train.append(i)
        for path in _train_test_split_paths( os.path.join(dp, dir_path), 'test'):
    ...
    return (x_train, y_train), (x_test, y_test), label_dirs
